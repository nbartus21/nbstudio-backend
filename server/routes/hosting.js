...
router.delete('/hosting/orders/:id', async (req, res) => {
  try {
    const order = await Hosting.findById(req.params.id);
    ...
    await HostingNotification.create({
      ...
      message: `${order.client.name} rendelése törölve lett`,
      severity: 'info'
    });
...
router.delete('/hosting/orders', async (req, res) => {
  try {
    // Minden rendelés lekérdezése
    const orders = await Hosting.find();
    ...
    await HostingNotification.create({
      ...
      title: 'Összes rendelés törölve',
      message: `${count} rendelés törölve lett`,
      severity: 'warning'
    });

    res.json({ message: `${count} rendelés sikeresen törölve` });
  } catch (error) {
